...
function DataTable<TData, TValue>({
  ...
  const onExportCsv = React.useCallback(() => {
    ...
    const headerLine = visibleColumns
      .map((column) =>
        escapeCsvValue(getHeaderLabel(column.columnDef.header, column.id)),
      )
      .join(",");
    ...
    <div className="space-y-3" data-slot="data-table">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {filters.map((filter) => {
          ...
          const options =
            filter.type === "select"
              ? resolveFilterOptions(filter.columnId, filter.options)
              : [];
            ...
            return (
              ...
              >
                ...
                <SelectContent align="end">
                  <SelectItem value={DATA_TABLE_ALL_VALUE}>
                    All - {filter.label}
                  </SelectItem>
                  {options.map((option) => (
          ...
          return (
            <Input
              ...
              placeholder={
                filter.placeholder ?? `Filter ${filter.label.toLowerCase()}...`
              }
              value={filterValue}
              onChange={(event) => {
                column.setFilterValue(
                  filter.parser
                    ? filter.parser(event.target.value)
                    : event.target.value,
                );
                table.setPageIndex(0);
              }}
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <p className="text-muted-foreground text-xs">
          Showing {table.getRowModel().rows.length} of {filteredRowsCount}{" "}
          filtered records ({data.length} total)
        </p>
