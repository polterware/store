-- RPC: bootstrap_first_admin (v3)
-- Robust version: drops previous overloads, recreates clean.
-- Auto-confirms email, checks for existing admin, assigns role.
-- SECURITY DEFINER — no auth.uid() required.
...
create function public.bootstrap_first_admin(p_user_email text)
returns boolean
language plpgsql
security definer
...
begin
  ...
  if v_user_id is null then
    raise exception 'User with email % not found in auth.users', p_user_email;
  end if;
