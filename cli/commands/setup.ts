...
export async function runSetup() {
  ...
  await execWithSpinner('Installing packages', 'pnpm install --frozen-lockfile')
  console.log()
  ...
  if (!linked) {
    console.log(pc.dim('  No linked project found. Running supabase link...\n'))
  ...
  if (envValid && (linked || existsSync('supabase/.temp/project-ref'))) {
    ...
    if (push) {
      console.log()
      const code = await execInteractive('supabase', ['db', 'push', '--linked'])
      if (code !== 0) {
